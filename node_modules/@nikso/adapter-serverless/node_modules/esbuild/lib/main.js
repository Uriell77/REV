...
function createChannel(streamIn) {
  ...
  let handleIncomingPacket = (bytes) => {
    if (isFirstPacket) {
      ...
      if (binaryVersion !== "0.10.1") {
        throw new Error(`Cannot start service: Host version "${"0.10.1"}" does not match binary version ${JSON.stringify(binaryVersion)}`);
      }
      return;
    }
...
var ensureServiceIsRunning = () => {
  ...
  let child = child_process.spawn(command, args.concat(`--service=${"0.10.1"}`, "--ping"), {
    windowsHide: true,
  ...
  longLivedService = {
    ...
    formatMessages: (messages, options) => {
      return new Promise((resolve, reject) => service.formatMessages("formatMessages", refs, messages, options, (err, res) => err ? reject(err) : resolve(res)));
    }
  };
  return longLivedService;
};
var runServiceSync = (callback) => {
  ...
  callback(service);
  let stdout = child_process.execFileSync(command, args.concat(`--service=${"0.10.1"}`), {
    cwd: defaultWD,
    windowsHide: true,
    input: stdin,
